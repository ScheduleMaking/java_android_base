/*
첫 번째 탭을 선탣했을 경우 실행되는 Activity.
[다음] 버튼을 클릭하면 nextData 메소드가 실행되며 다음 메모 내용이 제시된다.
[이전] 버튼을 클릭하면 previousData 메소드가실행되며 이전 메모 내용이 제시된다.
[삭제] 버튼을 클릭하면 deleteData 메소드가 실행되며 현재 보여지는 메모가 삭제된다.
[수정] 버튼을 클릭하면 modifyData 메소드가 실행되며 메모를 수정할 수 있도록 다른 Activity로 화면 전환한다.
 */
